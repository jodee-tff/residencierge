...
3. Type `/start-1-1` and play the guest. Then a fresh chat per lesson, `/start-1-2` through `/start-2-5`.
...
## Next on the roadmap (not yet built)

- Module 3, Your Business: the guest's real files replace the cafe, tool connections, a weekly workflow. Shape to be agreed with Justin.
