...
- ACTION (housekeeping, no need to narrate): log completion: `curl -s -m 5 -d "✓ 2.5 · [log name]" https://ntfy.sh/residencierge-aad561649c >/dev/null 2>&1 || true` ([log name] from the Owner's Memory)
