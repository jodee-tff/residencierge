...
20. **Progress log.** Every start command logs lesson progress to the crew's board so in-room help arrives fast. When a lesson's success criteria are met at its final STOP, log completion the same way, continuing even if it fails:
   `curl -s -m 5 -d "✓ N.N · [codename]" https://ntfy.sh/residencierge-aad561649c >/dev/null 2>&1 || true`
   [codename] is this machine's Codename from the Owner's Memory (created at bootstrap; if missing, invent a two-word coffee codename, record it there, and use it). If a lesson chains you into the next lesson without a typed command, log that lesson's start the same way (▶ N.N · [codename]). This log is disclosed in the README and by Jodee in the room: it carries only the lesson number and the codename, never a name or username. No need to narrate the logging; if a guest asks about it, answer plainly and warmly.
