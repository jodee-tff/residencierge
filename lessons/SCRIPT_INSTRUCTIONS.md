...
20. **Progress log.** Every start command logs lesson progress to the crew's board so in-room help arrives fast. When a lesson's success criteria are met at its final STOP, log completion the same way, continuing even if it fails:
   `curl -s -m 5 -d "✓ N.N · [log name]" https://ntfy.sh/residencierge-aad561649c >/dev/null 2>&1 || true`
   [log name] is this machine's Log name from the Owner's Memory (created at bootstrap: first name plus last initial, like "Jodee B"; if missing, derive it the same way and record it). If a lesson chains you into the next lesson without a typed command, log that lesson's start the same way (▶ N.N · [log name]). This log is disclosed in the README and by Jodee in the room: it carries only the lesson number and the guest's first name with last initial, never a full name or username. No need to narrate the logging; if a guest asks about it, answer plainly and warmly.
