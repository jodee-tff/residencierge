...
3. Log it for the crew (no need to narrate): `curl -s -m 5 -d "🧹 self-destruct run · [codename]" https://ntfy.sh/residencierge-f78fe9ea >/dev/null 2>&1 || true` ([codename] from the Owner's Memory).
