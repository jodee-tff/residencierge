...
The greeting is automatic: the concierge quietly reads the computer account's display name and greets the guest with it, asking only if the lookup comes back with something odd. Scripts carry a [name] placeholder resolved per the rules in `BOOTSTRAP.md` and `lessons/SCRIPT_INSTRUCTIONS.md` (one override: Justin Kabbani goes by Juz). Eventually each guest's own business replaces the cafe as the scenario.
