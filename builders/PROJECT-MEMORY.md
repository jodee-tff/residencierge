...
## Guest run 2: 30 August 2026 (automated, post-feedback build)

Fresh clone, claude-jk profile, deliberately awkward guest (cafe named "Steve", scrappy lowercase hunch, no model picker available). Stopped at end of 2.3. PASSED: dynamic name greeting, yes/no opener, folder-move teaching, WOMBAT, see-it-empty beats, the naming beat ("Steve it is. A cafe with a first name only, bold move"), own-words hunch recorded verbatim, the 1.3 hunch callback with verdict ("the truth was sneakier than I thought"), agents with evidence/interpretation/next-move structure, memory card carrying Cafe: Steve, memory proof in a fresh chat, node-missing handled with the crew line, the design table, and the build: theme B, Steve on the card, coffee-cup stamp iteration, all functional tests green. FIXED off the back of it: brand-name swap-proofing in both templates plus verify-no-trace lines in 2.2/2.3 (the Midnight Gold split markup missed the swap in previews), a models-picker-missing fallback in 1.4, and the 1.1 completion ping moved inline to the folder-move turn. Known cosmetic risk: about two em dash slips per full run in tutor-formatted lists, plus one pre-welcome narration line during the clone turn (the model checks the repo before adopting the persona; it reads as a safety check, arguably a feature). Timeouts during the run were a local DNS outage, not the course.
